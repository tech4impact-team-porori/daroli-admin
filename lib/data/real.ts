...
export const realDataSource: DataSource = {
  ...
  async listManagers(filter: ManagerFilter): Promise<ManagerWithStats[]> {
    ...
    return (res?.items || []).map((h) => ({
      ...
      recipientCount: h.assignedRecipientCount,
      monthlyLogCount: h.monthlyApprovedLogs,
      totalLogCount: h.accumulatedApprovedLogs,
      approvedTotal: h.accumulatedApprovedSettlement,
      paidTotal: h.accumulatedPaidWithdrawal,
      balance: h.withdrawableBalance,
    }));
  },
